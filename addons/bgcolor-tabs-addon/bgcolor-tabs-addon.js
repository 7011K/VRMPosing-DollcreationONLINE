...
window.MyAppAddons.push(async function({ threeRenderer, addonBaseUrl }) {
  ...
  function observeSettingsPanel() {
    ...
    function insertAddon() {
      ...
      if (!targetUl) return;
      if (targetUl.querySelector("[data-addon='bgcolor-tabs']")) return;

      // 並び順を「背景」「背景色変更」「UI表示」としたい
      const lis = Array.from(targetUl.children);
      ...
      if (uiDisplayIdx !== -1 && insertIdx > uiDisplayIdx) {
        insertIdx = uiDisplayIdx;
      }

      const { headerDiv, collapseDiv } = createAddonTab();

      if (insertIdx >= 0 && insertIdx < targetUl.children.length) {
        targetUl.insertBefore(headerDiv, targetUl.children[insertIdx]);
        targetUl.insertBefore(collapseDiv, targetUl.children[insertIdx + 1]);
      } else {
        targetUl.appendChild(headerDiv);
        targetUl.appendChild(collapseDiv);
      }
    }
